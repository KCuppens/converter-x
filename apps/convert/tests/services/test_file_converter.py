...
class FileConverterTestCase(CustomGraphQLTestCase):
    ...
    def test_convert_from_pptx_to_pdf(self):
        self.initial_file.file = "test_files/test_pptx.pptx"
        self.initial_file.save(update_fields=["file"])
        if sys.platform == "win32":
            assert True
        else:
            path = FileConverter().convert_from_pptx_to_pdf(self.conversion)
            self.assertTrue(path.split(".")[-1] == "pdf")
            self.assertTrue(os.path.exists(path))
            self.assertTrue(path)
            os.remove(path)
    ...
    def test_convert_from_wav_to_mp3(self):
        self.initial_file.file = "test_files/test_wav.wav"
        self.initial_file.save(update_fields=["file"])
        if sys.platform == "win32":
            assert True
        else:
            path = FileConverter().convert_from_wav_to_mp3(self.conversion)
            self.assertTrue(path.split(".")[-1] == "mp3")
            self.assertTrue(os.path.exists(path))
            self.assertTrue(path)
            os.remove(path)
